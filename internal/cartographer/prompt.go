...
- Include 'item_selector' — a CSS selector that matches EXACTLY ONE element per repeating item: the primary title/link the user would click. It MUST NOT match metadata links (subreddit names, author links, comment counts, domain labels). The final 'item_selector' MUST NOT contain '[data-mache-id]'. Use the most specific selector possible — prefer child combinators (>) and specific classes/attributes over broad descendant selectors. For example, if post titles share a path like "article.w-full > shreddit-post > a[slot=title]", use that exact pattern. A selector like "article a.block" is TOO BROAD — it matches every link in the article. The 'Path' field for each element is already a CSS selector. For 'item_selector', derive the most concise and precise CSS selector targeting the primary item. Begin by refining the item's 'Path': remove '[data-mache-id]', 'id', unique 'href' attributes, or any other instance-specific or dynamically generated attributes. Identify the repeating element's root in the refined Path (e.g., 'article.post') and construct the item_selector from that root to the primary item, retaining only the minimal set of ancestors necessary for unique identification. Prioritize specific classes/attributes on the primary item and convert descendant selectors (spaces) to child combinators (>) where possible. The selector must exclusively match the primary item within its repeating block, avoiding metadata links. Leave empty for non-list zones or when no clear repeating pattern exists.

For each zone, select the single element (and its data-mache-id) from the provided list that serves as its primary anchor. Prioritize a clear structural parent element if available; otherwise, select the visually highest and leftmost interactive element.
