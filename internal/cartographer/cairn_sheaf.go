...
// distributions (two tight clusters far apart). With bimodal data,
// the median is dominated by cross-cluster distances, so 0.5×median
// can still be large enough to merge distinct clusters. The 25th
// percentile captures the intra-cluster distance scale directly.
func computeAdaptiveThreshold(stalks [][]float64, weights [CairnNumDims]float64) float64 {
	n := len(stalks)
	if n < 2 {
		return 1.0
	}
	...
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			var sqDiff float64
			for d := 0; d < CairnNumDims; d++ {
				diff := weights[d] * (stalks[i][d] - stalks[j][d])
				sqDiff += diff * diff
			}
			dists = append(dists, sqDiff)
		}
	}
	...
	// Sort (bubble sort is fine for ≤21 pairs from 7 zones)
	...
	// 25th percentile: captures intra-cluster distance scale
	p25Idx := len(dists) / 4
	threshold := dists[p25Idx]

	// Floor: don't merge zones that are truly different
	if threshold < 1e-6 {
		threshold = 1e-6
	}
